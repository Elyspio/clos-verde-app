...
### PR Threads and Comments (via `az devops invoke`)

`az repos pr` does not provide direct subcommands to list PR threads/comments.
Use `az devops invoke` with Git preview resources:

```bash
# List threads on a PR
az devops invoke \
  --area git \
...
# List comments for one thread
az devops invoke \
  --area git \
  --resource pullRequestThreadComments \
  --org https://dev.azure.com/{org} \
  --route-parameters project={project} repositoryId={repo} pullRequestId={pr-id} threadId={thread-id} \
  --http-method GET \
  --api-version 5.1-preview \
  -o json
```

Filter active (open) comment threads:
...
If your local extension/version rejects this resource/api pair, fall back to the direct REST endpoint:

```bash
GET https://dev.azure.com/{org}/{project}/_apis/git/repositories/{repositoryId}/pullRequests/{pullRequestId}/threads?api-version=7.1-preview.1
```

For write operations on PR comments/threads (create inline thread, general comment, reply, status update), use direct REST calls from PowerShell:

```powershell
# Auth headers from current az login
...
$repo = "{repo}"
$prId = "{pr-id}"
$baseUrl = "https://dev.azure.com/$org/$project/_apis/git/repositories/$repo/pullRequests/$prId"

# 1) Create general PR comment thread
$generalBody = @{
  comments = @(@{ parentCommentId = 0; content = "General PR comment"; commentType = 1 })
  status = "active"
} | ConvertTo-Json -Depth 4
Invoke-RestMethod -Uri "$baseUrl/threads?api-version=6.0" -Method POST -Headers $headers -Body $generalBody

# 2) Create inline PR comment thread
$inlineBody = @{
  comments = @(@{ parentCommentId = 0; content = "Inline comment"; commentType = 1 })
  status = "active"
  threadContext = @{
...
Invoke-RestMethod -Uri "$baseUrl/threads?api-version=6.0" -Method POST -Headers $headers -Body $inlineBody

# 3) Reply to existing thread
$threadId = 12345
$replyBody = @{ content = "Reply text"; parentCommentId = 1; commentType = 1 } | ConvertTo-Json
Invoke-RestMethod -Uri "$baseUrl/threads/$threadId/comments?api-version=6.0" -Method POST -Headers $headers -Body $replyBody

# 4) Update thread status (active|fixed|wontFix|closed|byDesign|pending)
$statusBody = @{ status = "fixed" } | ConvertTo-Json
Invoke-RestMethod -Uri "$baseUrl/threads/$threadId`?api-version=6.0" -Method PATCH -Headers $headers -Body $statusBody
```
